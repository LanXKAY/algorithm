...
<div><div>Related Topics</div><div><li>双指针</li><li>字符串</li></div></div><br><div><li>👍 303</li><li>👎 0</li></div>
*/
...
class CompareVersion_165 {
    public int compareVersion(String version1, String version2) {
		...
		int pos = 0;
		while (pos < items1.length && pos < items2.length) {
			int tmp1 = Integer.parseInt(items1[pos]);
			int tmp2 = Integer.parseInt(items2[pos]);
		...
		while (pos < items1.length) {
			int tmp = Integer.parseInt(items1[pos++]);
			if (tmp > 0) {
				return 1;
			}
		}

		while (pos < items2.length) {
			int tmp = Integer.parseInt(items2[pos++]);
			if (tmp > 0) {
				return -1;
			}
		}
